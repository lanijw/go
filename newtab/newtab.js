...
document.getElementById('shorthand-input').onkeydown = async e => {
  if (e.key === 'Enter') {
    ...
    const matchingMapping = mapping[mapKey].filter(m => m[InputType.SHORT] !== '').find(m => shorthand.startsWith(m[InputType.SHORT]));
    if (matchingMapping) {
      const redirectUrl = matchingMapping[InputType.LONG] + shorthand.slice(matchingMapping[InputType.SHORT].length);
